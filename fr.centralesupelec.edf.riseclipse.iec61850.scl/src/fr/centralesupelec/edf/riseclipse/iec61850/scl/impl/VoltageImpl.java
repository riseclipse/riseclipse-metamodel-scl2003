...
public class VoltageImpl extends ValueWithUnitImpl implements Voltage {
    ...
    * <!-- end-user-doc -->
	 ...
	 * @generated
	 */
	public boolean validateVoltage_unit_value_fixed(final DiagnosticChain diagnostics, final Map<Object, Object> context) {
		/**
		 *
		 * inv Voltage_unit_value_fixed:
		 *   let
		 *     severity : Integer[1] = 'Voltage::Voltage_unit_value_fixed'.getSeverity()
		 *   in
		 *     if severity <= 0
		 *     then true
		 *     else
		 *       let
		 *         result : OclAny[1] = let status : Boolean[?] = self.unit <> null implies self.unit = 'V'
		 *         in
		 *           if status = true
		 *           then true
		 *           else
		 *             Tuple{message = 'unit attribute shall be V in Voltage (line ' +
		 *               self.lineNumber.toString() + '). ' + 'Current value is ' +
		 *               self.unit.toString(), status = status
		 *             }
		 *           endif
		 *       in
		 *         'Voltage::Voltage_unit_value_fixed'.logDiagnostic(self, null, diagnostics, context, null, severity, result, 0)
		 *     endif
		 */
		final /*@NonInvalid*/ Executor executor = PivotUtilInternal.getExecutor(this);
		final /*@NonInvalid*/ IntegerValue severity_0 = CGStringGetSeverityOperation.INSTANCE.evaluate(executor, SclTables.STR_Voltage_c_c_Voltage_unit_value_fixed);
		...
		if (le) {
			...
			try {
				...
				final /*@NonInvalid*/ boolean ne = unit != null;
				/*@NonInvalid*/ boolean status;
				if (ne) {
					final /*@NonInvalid*/ boolean eq = SclTables.STR_V.equals(unit);
					status = eq;
				}
				else {
				...
				if (status) {
					...
					final /*@NonInvalid*/ String sum = StringConcatOperation.INSTANCE.evaluate(SclTables.STR_unit_32_attribute_32_shall_32_be_32_V_32_in_32_Voltage_32_o_line_32, toString);
					...
					final /*@Thrown*/ String toString_0 = OclAnyToStringOperation.INSTANCE.evaluate(unit);
			...
			final /*@NonInvalid*/ boolean logDiagnostic = CGStringLogDiagnosticOperation.INSTANCE.evaluate(executor, TypeId.BOOLEAN, SclTables.STR_Voltage_c_c_Voltage_unit_value_fixed, this, (Object)null, diagnostics, context, (Object)null, severity_0, CAUGHT_symbol_1, SclTables.INT_0).booleanValue();
