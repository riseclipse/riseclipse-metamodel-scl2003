...
public class AsdXMLHandler extends SAXXMLHandler {
    ...
    public AsdXMLHandler( XMLResource xmiResource, XMLHelper helper, Map< ?, ? > options, MyEObjectStack parentObjects ) {
        ...
        this.objects = parentObjects;
    }
    
    public AsdXMLHandler( XMLResource xmiResource, XMLHelper helper, Map< ?, ? > options ) {
        super( xmiResource, helper, options );
    }
    ...
    protected EPackage getPackageForURI( String uriString ) {
        if( ! AsdPackage.eNS_URI.equals( uriString )) {
//            AbstractRiseClipseConsole.getConsole().info( "ignoring namespace " + uriString );
    ...
    protected void processObject( EObject object ) {
        if( lineNumbers.empty() ) {
            ...
            if( object instanceof AsdObject ) {
                (( AsdObject ) object ).setLineNumber( lineNumber );
                (( AsdObject ) object ).setFilename( resourceURI.lastSegment() );
            }
        }
        // TODO: error message
    ...
    protected void setFeatureValue( EObject object, EStructuralFeature feature, Object value, int position ) {
        ...
        if(( feature.getUpperBound() == 1 ) && object.eIsSet( feature )) {
            AbstractRiseClipseConsole.getConsole().error(
                      XML_HANDLER_CATEGORY, resourceURI.lastSegment(), (( AsdObject ) object ).getLineNumber(),
    ...
    protected String getXSIType() {
        if (privateRootName != null) {
            String xsiType = privateRootName;
            privateRootName = null;
            return xsiType;
        }
